...
class TFEmbeddings(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        if config.sinusoidal_pos_embds:
            position_enc = np.array([[pos / np.power(10000, 2 * (j // 2) / config.dim) for j in range(config.dim)] for pos in range(config.max_position_embeddings)])
            self.position_embeddings = tf.keras.layers.Embedding(
                config.max_position_embeddings,
                config.dim,
                embeddings_initializer=tf.keras.initializers.Constant(position_enc),
